...
function count(a){
	if(a.length<141){
 		b=140-a.length;
		c=document.getElementById("count").innerHTML="还可以输入"+b+"字";
 		return c;
 		}
 		else{
		d=a.length-140;
		e=document.getElementById("count").innerHTML="已超过"+d+"字";
 			return e;
			}
  }
...
function delete_fre(fre_id){
		...
		url=url+"?fresh="+fre_id
		xmlHttp.onreadystatechange=function(){stateChangedc()}; 
		xmlHttp.open("GET",url,true)
		xmlHttp.send(null)
}
