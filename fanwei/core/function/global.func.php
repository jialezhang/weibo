 ...
 * by fzmatthew
 */
function getImgName($img_url,$width=0,$height=0,$gen = 0)
{
	static $imagec = NULL;
	if($width>0&&$height>0)
	{
...
function deleteShareImg($img_path,$sizes = array())
{
	static $size_setting = NULL;
	if($sizes !== false && empty($sizes))
	{
		if($size_setting === NULL)
		{
	...
	foreach($sizes as $size)
	{
		if($size[0] > 0 || $size[1] > 0)
		{
			@unlink(str_replace('.'.$paths['extension'],'',$img_path).'_'.$size[0].'x'.$size[1].'.jpg');
		}
	}
}
?>