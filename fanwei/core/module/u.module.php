...
//		$share_list_html = tplFetch("inc/u/me_share_list",$args);
		...
		$link_url = $_FANWE['site_url']."services/service.php?m=user&a=shareSelf&width=190&p=1&home_uid=".$home_uid;
	...
	public function fav()
	{
		...
		$link_url = $_FANWE['site_url']."services/service.php?m=user&a=shareFav&width=190&p=1&home_uid=".$home_uid;
	...
	public function message()
	{
		...
		$pager = buildPage('u/'.ACTION_NAME,array(),$count,$_FANWE['page'],1);
	...
	public function album()
	{
		...
		switch($type)
		{
			...
			default:
				...
				{
					$args['type'] = 1;
					$pager = buildPage('u/album',$args,$home_user['albums'],$_FANWE['page'],9);
					$sql = 'SELECT * FROM '.FDB::table('album').' 
					...
					{
						$data['imgs'] = array();
						$img_sql = "select sp.img as img from ".FDB::table("share_photo")." as sp left join ".FDB::table("album_share")." as als on als.share_id = sp.share_id left join ".FDB::table("album")." as a on a.id = als.album_id where a.id = ".$data['id'];
