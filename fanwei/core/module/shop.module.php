...
class ShopModule
{
	public function index()
	{
		...
		{
			$cache_data = unserialize($data['data']);
			$data['tags'] = array();
			if($cache_data)
			{
	...
	public function show()
	{
		...
		$cache_data = unserialize($shop['data']);
		...
		$page_args['id'] = $id;
		$sql = 'SELECT COUNT(DISTINCT share_id) FROM '.FDB::table('share_goods').' WHERE shop_id = '.$id;
		...
		$share_ids = array();
		$sql = 'SELECT DISTINCT share_id FROM '.FDB::table('share_goods').' WHERE shop_id = '.$id.' ORDER BY share_id DESC LIMIT '.$pager['limit'];
		$res = FDB::query($sql);
		while($data = FDB::fetch($res))
		{
