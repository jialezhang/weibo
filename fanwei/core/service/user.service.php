...
class UserService
{
	...
	public function usersFormat(&$uid_list)
	{
		...
		if(count($uids) > 0)
		{
			...
			if(count($diffs) > 0)
			{
				...
				$res = FDB::query("SELECT uid,user_name,reg_time,uc.* FROM ".FDB::table('user').' 
	...
	{
		$uid = (int)$uid;
		return (int)FDB::resultFirst('SELECT num FROM '.FDB::table('user_statistics').' WHERE uid = '.$uid.' AND type = 7');
	}
