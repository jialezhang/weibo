...
class StyleCategoryAction extends CommonAction
{
	...
	public function add()
	{
		$cate_list = D("StyleCategory")->where('status = 1')->field('cate_id,parent_id,cate_name')->order('sort ASC,cate_id ASC')->findAll();
	...
	public function edit()
	{
		...
		$childs[] = $id;
		$cate_list = D("StyleCategory")->where('status = 1 && cate_id not in ('.implode(',', $childs).')')->field('cate_id,parent_id,cate_name')->order('sort ASC,cate_id ASC')->findAll();
		$cate_list = D("StyleCategory")->toFormatTree($cate_list,array('cate_name'),'cate_id');
